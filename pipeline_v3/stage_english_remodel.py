...
class EnglishRemodeler:
    def __init__(
        ...
        translator: Optional[StageTranslator] = None,
    ) -> None:
        self.core = core
        ...
        # Optional StageTranslator for downstream dialect pipeline.
        # If not provided, translation methods will raise RuntimeError.
        self.translator: Optional[StageTranslator] = translator
    ...
    # ------------------------------------------------------------------
    # Translation helpers (require translator to be set)
    # ------------------------------------------------------------------

    def _require_translator(self) -> StageTranslator:
        if self.translator is None:
            raise RuntimeError(
                "No StageTranslator provided. Pass translator=StageTranslator(...) "
                "when constructing EnglishRemodeler to use translation methods."
            )
        return self.translator

    def remodel_to_tamil(
        self, user_query: str, raw_answer: str, profile: Dict[str, Any]
    ) -> str:
        """Remodel English answer, then translate it to Tamil via OpenAI."""
        translator = self._require_translator()
        remodeled_english = self.remodel(user_query, raw_answer, profile)
        return translator.english_to_tamil(remodeled_english, profile)

    def remodel_to_thenitamil(
        self, user_query: str, raw_answer: str, profile: Dict[str, Any]
    ) -> str:
        """Remodel English, translate to Tamil, then convert to Theni Tamil dialect."""
        translator = self._require_translator()
        remodeled_english = self.remodel(user_query, raw_answer, profile)
        tamil_text = translator.english_to_tamil(remodeled_english, profile)
        return translator.tamil_to_thenitamil(tamil_text)
